...
class TabularSiameseDataset(TabularPandas):

    def __init__(self, *args, **kwargs):
        super(TabularSiameseDataset, self).__init__(*args, **kwargs)

    def __getitem__(self, i):
        import ipdb; ipdb.set_trace()
        # get the ith item
        row1_cats = tensor(
            self.train.dataloaders().cats.iloc[i].values)
        row1_conts = tensor(
            self.train.dataloaders().conts.iloc[i].values)
    ...
    def _draw(self, i):
        cls = self.train.iloc[i][self.targ.columns[0]]
        same = random.random() < 0.5
        if not same:
            cls = random.choice(
                [label for label in set(self.y) if label != cls])
    ...
    def get_items(self, i, j, show=False):
        ...
        x1_cls = self.vocab[self.train.y.iloc[i]]

        x2, _, _ = self.__getitem__(j)
        x2_cls = self.vocab[self.train.y.iloc[j]]
