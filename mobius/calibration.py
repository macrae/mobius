...
class ModelWithTemperature(nn.Module):
    ...
    def set_temperature(self, valid_loader):
        ...
        labels_list = []
        with torch.no_grad():
            for cats, conts, label in valid_loader:
                input = (cats, conts)
                logits = self.model(*input)
        ...
        before_temperature_nll = nll_criterion(
            logits, labels.squeeze().long()).item()
        before_temperature_ece = ece_criterion(
            logits, labels.squeeze().long()).item()
        ...
        after_temperature_nll = nll_criterion(
            self.temperature_scale(logits), labels.squeeze().long()).item()
